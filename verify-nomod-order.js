...
// whatever's actually left, in case it changed since checkout (see the
// file header for the known, disclosed limitation this doesn't fully
// close). Called from inside the same status !== 'paid' guard that
// already makes the rest of this confirmation step run exactly once
// per order, so this can't double-deduct on a repeat verification call.
