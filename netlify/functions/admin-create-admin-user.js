...
exports.handler = async function (event) {
  ...
  try {
    var existingRes = await fetch(
      `${SUPABASE_URL}/rest/v1/admin_users?username=eq.${encodeURIComponent(username)}&select=id`,
    ...
    var res = await fetch(`${SUPABASE_URL}/rest/v1/admin_users`, {
      method: 'POST',
      headers: supabaseHeaders(),
      body: JSON.stringify({
        username: username,
    ...
    return {
      statusCode: 200,
      body: JSON.stringify({ user: { id: user.id, username: user.username, display_name: user.display_name, active: user.active, created_at: user.created_at } })
    };
  } catch (e) {
