// netlify/functions/remove-from-wishlist.js
//
// POST { visitorToken, sareeId }
// - Mirrors remove-from-cart.js exactly, against the wishlist_items table.
...
exports.handler = async function (event) {
  ...
  try {
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/wishlist_items?email=eq.${encodeURIComponent(payload.email)}&saree_id=eq.${encodeURIComponent(sareeId)}`,
      {
        method: 'DELETE',
        headers: {
